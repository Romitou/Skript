...
public abstract class EntityData<E extends Entity> implements SyntaxElement {// TODO extended horse support, zombie villagers // REMIND unit
	...
	public static YggdrasilSerializer<EntityData> serializer = new YggdrasilSerializer<EntityData>() {
		@Override
		public void serialize(YggdrasilWriter writer, EntityData entityData) {
			writer.writeString("codeName", entityData.info.codeName);
			if (!entityData.isPlural().isUnknown())
				writer.writeBoolean("plural", entityData.isPlural().isTrue());
			if (!entityData.isBaby().isUnknown())
				writer.writeBoolean("baby", entityData.isBaby().isTrue());
		}

		@Override
		public @Nullable EntityData deserialize(YggdrasilReader reader) {
			...
			if (info == null)
				return null;
			try {
				final EntityData<?> entityData = info.c.newInstance();
				if (reader.contains("plural"))
					entityData.setPlural(Kleenean.get(reader.readBoolean("plural")));
				if (reader.contains("baby"))
					entityData.setBaby(Kleenean.get(reader.readBoolean("baby")));
				return entityData;
...
//		}
	...
	public void setBaby(Kleenean baby) {
		this.baby = baby;
	}
}
